...
class TextEncoder(nn.Module):
    ...
    def apply_text_to_template(text, template, prevent_empty_text=True):
     ...
     def text2tokens(self, text, prompt_template, image1=None, image2=None, clip_text_override=None):
        """
        Tokenize the input text, also processing image inputs if provided for a VLM model.

        Args:
            text (str or list): Input text.
            prompt_template (dict): Template for formatting the prompt if VLM.
            image1 (Image, optional): First input image (if using a VLM).
            image2 (Image, optional): Second input image (if using a VLM).
            clip_text_override (str, optional): Text to use for CLIP model (if applicable).
        """
        tokenize_input_type = "str"
        # For VLM, prepare image inputs and include <image> token if images are provided
        if self.text_encoder_type == "vlm":
            raw_images = []
            if image1 is not None:
                raw_images.append(image1.squeeze(0)*255)
                text = text.replace("<image>", "<image><image>")  # Add two <image> tokens
            if image2 is not None:
                raw_images.append(image2.squeeze(0)*255)
                text = text.replace("<image>", "<image><image>", 1) # Replace only the first occurrence for image2

        # Apply prompt template for LLMs and VLMs if provided
        ...
        if tokenize_input_type == "str":
            if self.text_encoder_type == "vlm":
                text_tokens = self.processor(
                    ...
                    text, 
                    **kwargs,
                    ).to(self.device, dtype=self.dtype) #added device
            else:
    ...
    def encode(
        ...
        device=None,
        max_context_length=256,  # Add max_context_length here
    ):
        """
        Args:
